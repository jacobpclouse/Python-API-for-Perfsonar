...
def writeOutToFile(outgoingData,currentDatetime,filenamePrefix,nodeKey):
    with open(f'{nodeKey}__{filenamePrefix}{currentDatetime}.json', 'a') as z:
...
def loopThroughArray(inputArray,targetTestResults,targetTestPrefix):
    ...
    for elements in inputArray:
        ...
        # insert into specific URI and print
        uri = f'http://192.168.50.190/esmond/perfsonar/archive/{elements}{targetTestResults}'
        print(uri)

        # getting data from uri - packet trace
        data = openApiUri(elements,targetTestResults)

        # write out to file  
        writeOutToFile(data,use_this_datetime,targetTestPrefix,elements)
...
'''
# get failures 
loopThroughArray(singleMetaKey,failuresURI,failuesPrefix)

# get packet trace 
loopThroughArray(singleMetaKey,packetTraceURI,packetTracePrefix)

# get path-mtu 
loopThroughArray(singleMetaKey,pathMtuURI,pathMtuPrefix)

# get pscheduler 
loopThroughArray(singleMetaKey,pschedulerURI,pschedulerPrefix)
